...
const dir = resolve(process.cwd(), "data", "captures-dydx", new Date().toISOString().slice(0, 10));
const path = resolve(dir, `${market}.ws.jsonl`);
if (!existsSync(path)) { console.error(`no WS capture at ${path} — run: npm run record:dydx`); process.exit(1); }
