...
const TOP = num("--top", 60), DAYS = num("--days", 30);
...
## How to copy (if copyable)
${s.copyabilityVerdict === "un-copyable"
      ? "Do NOT copy — the edge is latency/fee-rebate that a follower can't capture. Watch only."
      : `Mirror ${s.directionality === "momentum-short" ? "short" : "long"} entries on ${s.topCoin}${s.nCoins > 1 ? " and its other coins" : ""} with ~${fmtHold(s.medianHoldMs)} expected hold; size as a fraction of their notional, model slippage, and replicate exits within the same window. Paper-track first (\`npm run hl:copy-paper\`).`}
${drift}
---
...
for (const w of ranked) {
  try {
    ...
    const snap: WalletSnapshot = {
      ...
      copyabilityScore: prof.copyability.score, copyabilityVerdict: prof.copyability.verdict,
...
// ranked index — copyable + verified first, the actual follow-list
index.sort((a, b) => (Number(b.verified) - Number(a.verified)) || (b.copyabilityScore - a.copyabilityScore));
const row = (s: WalletSnapshot) => `| [\`${s.address.slice(0, 10)}…\`](./${s.address}.md) | ${s.copyabilityVerdict} | ${s.verified ? "✓" : "·"}${s.flowDistorted ? " ⚠flow" : ""} | ${s.label} | ${fmtHold(s.medianHoldMs)} | ${usd(s.realizedPnl)} | ${f0(s.tradesPerDay, 1)} |`;
const followable = index.filter((s) => s.copyabilityVerdict !== "un-copyable" && s.verified && !s.flowDistorted);
...
## The follow-list — copyable AND verified-profitable AND clean capital flow (${followable.length})
${followable.length ? `| wallet | copyability | flags | strategy | median hold | realized | trades/day |\n|---|---|---|---|---|---|---|\n${followable.map(row).join("\n")}` : "_None this run — the top leaderboard is dominated by un-copyable HFT/MM. This is the honest result, not a failure._"}

## All tracked wallets (copyable + verified first)
| wallet | copyability | flags | strategy | median hold | realized | trades/day |
|---|---|---|---|---|---|---|
${index.map(row).join("\n")}

_✓ = verified-profitable on own fills · ⚠flow = withdrew >25% of equity (ROI distorted) · "un-copyable" = HFT/MM, edge is latency you can't replicate._
`;
writeFileSync(resolve(DOCS, "README.md"), readme);
const totalSnaps = store.snapshotCount();
store.close();

const v = (k: string) => index.filter((s) => s.copyabilityVerdict === k).length;
console.log(`\n  tracked ${index.length} · copyable ${v("copyable")} · hard ${v("hard")} · un-copyable ${v("un-copyable")}`);
console.log(`  follow-list (copyable+verified+clean): ${followable.length}`);
